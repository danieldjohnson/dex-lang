...
module Syntax (
    ...
    Expr (..), Atom (..), ArrowP (..), Arrow, PrimTC (..), Abs (..), Match (..),
...
-- A Match instance represents a pattern-match result as an atom.
data Match = MatchVar Var         -- Match a var.
           | MatchPairFst Match   -- Match PairCon, extract fst.
           | MatchPairSnd Match   -- Match PairCon, extract snd.
           | MatchNewtype Type Match    -- Match NewtypeCon, extract its value.
                                        -- The type is used for typechecks but
                                        -- not validated during matching.
           | MatchFail Atom       -- A failed match; this atom does not
                                  -- satisfy the containing constraints.
             deriving (Show, Eq, Generic)
...
instance HasVars Atom where
  freeVars atom = case atom of
    ...
    Match match -> matchFreeVars match
  ...
  subst env atom = case atom of
    ...
    Match match -> substMatch env match

matchFreeVars :: Match -> Scope
matchFreeVars match = case match of
  MatchVar v -> varFreeVars v
  MatchPairFst m -> matchFreeVars m
  MatchPairSnd m -> matchFreeVars m
  MatchNewtype ty m -> freeVars ty <> matchFreeVars m
  MatchFail atom -> freeVars atom

-- Substitute a variable into a match. If the match is successful, produces an
-- atom corresponding to the matched value. If the match fails, rewrites to a
-- MatchFail. If we end up with leftover names, the are passed through.
substMatch :: ScopedSubstEnv -> Match -> Atom
substMatch env match = case match of
    MatchVar v -> case substVar env v of
      Var v' -> Match $ MatchVar v'
      atom -> atom
    MatchPairFst inner -> case substMatch env inner of
      Con (PairCon x _) -> x
      atom -> fallthrough MatchPairFst atom
    MatchPairSnd inner -> case substMatch env inner of
      Con (PairCon _ x) -> x
      atom -> fallthrough MatchPairSnd atom
    MatchNewtype ty inner -> case substMatch env inner of
      Con (NewtypeCon _ x) -> x
      atom -> fallthrough (MatchNewtype $ subst env ty) atom
    MatchFail atom -> Match $ MatchFail $ subst env atom
  where
