...
data Match = MatchPairFst Atom   -- Match PairCon, extract fst.
           | MatchPairSnd Atom   -- Match PairCon, extract snd.
           | MatchNewtype Type Atom   -- Match NewtypeCon, extract its value.
...
matchFreeVars match = case match of
  MatchPairFst a -> freeVars a
  MatchPairSnd a -> freeVars a
  MatchNewtype ty a -> freeVars ty <> freeVars a

-- Substitute a variable into a match. If the match is successful, produces an
-- atom corresponding to the matched value; otherwise, remains unevaluated.
substMatch :: ScopedSubstEnv -> Match -> Atom
substMatch env match = case match of
  MatchPairFst inner -> case subst env inner of
    Con (PairCon x _) -> x
    atom -> Match $ MatchPairFst atom
  MatchPairSnd inner -> case subst env inner of
    Con (PairCon _ x) -> x
    atom -> Match $ MatchPairSnd atom
  MatchNewtype ty inner -> case subst env inner of
    Con (NewtypeCon _ x) -> x
    atom -> Match $ MatchNewtype ty atom
